...
static ui_wifi_list_use_ap_cb_t s_use_ap_cb;
static void *s_use_ap_user_data;
...
{
    s_use_ap_cb = cb;
    s_use_ap_user_data = user_data;
    ui_wifi_log((cb != NULL) ? "USE_AP callback attached" : "USE_AP callback detached");
}

static void ui_wifi_set_use_ap_btn_enabled(bool enabled)
{
    if(s_ctx.use_ap_btn == NULL) {
        return;
    }

    if(enabled) {
        lv_obj_clear_state(s_ctx.use_ap_btn, LV_STATE_DISABLED);
    } else {
        lv_obj_add_state(s_ctx.use_ap_btn, LV_STATE_DISABLED);
    }
}

/* Block scan only during transition states to avoid scan/connect overlap. */
static bool ui_wifi_conn_busy_for_scan(void)
{
    wifi_connection_snapshot_t snapshot;

    if(!wifi_connection_service_get_snapshot(&snapshot)) {
        return false;
    }

    /* Keep scan available in CONNECTED state so user can browse/switch AP. */
    return (snapshot.state == WIFI_CONN_STATE_CONNECTING) ||
           (snapshot.state == WIFI_CONN_STATE_RECONNECT_WAIT) ||
...
/* Rebuild AP rows after scan state changes (scan start/done/error). */
...
static void ui_wifi_scan_click_cb(lv_event_t *e)
{
    ...
    if(ui_wifi_conn_busy_for_scan()) {
        lv_label_set_text(s_ctx.hint_label,
                          "Scan is temporarily disabled while WiFi is connecting/reconnecting.");
        ui_wifi_log("SCAN_BLOCKED conn_busy");
        return;
    }
    ...
    ui_wifi_set_use_ap_btn_enabled(false);
...
/* Copy currently selected AP to profile page via shell callback. */
static void ui_wifi_use_selected_ap_cb(lv_event_t *e)
{
    ...
    if((aps == NULL) || (count == 0U) || (s_ctx.selected_idx >= count)) {
        ...
        ui_wifi_log("USE_AP_FAIL no_selection");
        return;
    }

    if(s_use_ap_cb == NULL) {
        ui_wifi_log("USE_AP_FAIL callback_missing");
        return;
    }

    s_use_ap_cb(&aps[s_ctx.selected_idx], s_use_ap_user_data);
    lv_label_set_text(s_ctx.hint_label, "AP copied to profile page.");
    ui_wifi_log("USE_AP_OK");
}

/* Timer keeps UI non-blocking while scan callback runs in background. */
static void ui_wifi_poll_timer_cb(lv_timer_t *timer)
{
    uint16_t count = 0U;
    bool conn_busy = ui_wifi_conn_busy_for_scan();
    ...
    if(wifi_scan_service_process(&s_ctx.service)) {
        ...
        ui_wifi_set_use_ap_btn_enabled(true);
        (void)wifi_scan_service_get_list(&s_ctx.service, &count);
        ...
        if((s_ctx.service.scan_sequence == 0U) && (count == 0U)) {
            ...
            lv_label_set_text(s_ctx.hint_label, "Select AP row, then tap Use This AP.");
        }
...
lv_obj_t *ui_wifi_list_page_create(lv_obj_t *menu)
{
    ...
    s_ctx.page = lv_menu_page_create(menu, "WiFi Scan");
    ...
    lv_label_set_text(title_label, "WiFi Scan List (EP07)");
    ...
    lv_obj_set_size(s_ctx.scan_btn, 140, 38);
    ...
    lv_obj_t *scan_btn_label = lv_label_create(s_ctx.scan_btn);
    lv_label_set_text(scan_btn_label, LV_SYMBOL_REFRESH " Scan WiFi");
    lv_obj_set_style_text_color(scan_btn_label, lv_color_hex(0xFFFFFF), LV_PART_MAIN);
    lv_obj_set_style_text_font(scan_btn_label, &lv_font_montserrat_14, LV_PART_MAIN);
    lv_obj_center(scan_btn_label);

    s_ctx.use_ap_btn = lv_button_create(scan_row);
    lv_obj_set_size(s_ctx.use_ap_btn, 160, 38);
    lv_obj_set_style_bg_color(s_ctx.use_ap_btn, lv_color_hex(0x0E7490), LV_PART_MAIN);
    lv_obj_set_style_border_color(s_ctx.use_ap_btn, lv_color_hex(0x67E8F9), LV_PART_MAIN);
    lv_obj_set_style_border_width(s_ctx.use_ap_btn, 1, LV_PART_MAIN);
    lv_obj_set_style_radius(s_ctx.use_ap_btn, 10, LV_PART_MAIN);
    lv_obj_add_event_cb(s_ctx.use_ap_btn, ui_wifi_use_selected_ap_cb, LV_EVENT_CLICKED, NULL);

    lv_obj_t *use_ap_btn_label = lv_label_create(s_ctx.use_ap_btn);
    lv_label_set_text(use_ap_btn_label, "Use This AP");
    lv_obj_set_style_text_color(use_ap_btn_label, lv_color_hex(0xFFFFFF), LV_PART_MAIN);
    lv_obj_set_style_text_font(use_ap_btn_label, &lv_font_montserrat_14, LV_PART_MAIN);
    lv_obj_center(use_ap_btn_label);
