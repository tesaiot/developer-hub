...
if [ -f "$BUNDLED_LIBS/libtrustm.so" ]; then
    USE_BUNDLED_LIBS=1
    echo "[CSR-RUNNER] Found bundled libtrustm.so"
fi
...
if [ -f "$BUNDLED_LIBS/trustm_provider.so" ]; then
    ...
    echo "[CSR-RUNNER] Using bundled trustm_provider.so"
...
if [ -z "$TRUSTM_PROVIDER" ]; then
    ...
    echo "[CSR-RUNNER] WARNING: Could not find trustm_provider.so"
fi

echo "[CSR-RUNNER] Using trustm tools from: $TRUSTM_LIB_PATH"
echo "[CSR-RUNNER] Using trustm_provider: $TRUSTM_PROVIDER"
...
if [ $USE_BUNDLED_LIBS -eq 1 ]; then
    ...
    echo "[CSR-RUNNER] Using bundled libraries from: $BUNDLED_LIBS"
fi

if [ $USE_SHIM -eq 1 ]; then
    echo "[CSR-RUNNER] Using OPTIGA shim for Paho MQTT TLS (default)"
    echo "[CSR-RUNNER] Shim path: $SHIM_PATH"
    echo "========================================"
    if [ $USE_BUNDLED_LIBS -eq 1 ]; then
        sudo -E LD_LIBRARY_PATH="$BUNDLED_LIBS:$LD_LIBRARY_PATH" LD_PRELOAD="$SHIM_PATH" TRUSTM_OPENSSL_CONF="$OPENSSL_CONF" TRUSTM_DATA_PATH="$TRUSTM_DATA_PATH" TRUSTM_LIB_PATH="$TRUSTM_LIB_PATH" "$DEPLOY_DIR/bin/tesaiot_csr_client" $ARGS
    else
        sudo -E LD_PRELOAD="$SHIM_PATH" TRUSTM_OPENSSL_CONF="$OPENSSL_CONF" TRUSTM_DATA_PATH="$TRUSTM_DATA_PATH" TRUSTM_LIB_PATH="$TRUSTM_LIB_PATH" "$DEPLOY_DIR/bin/tesaiot_csr_client" $ARGS
    fi
else
    echo "[CSR-RUNNER] Running without shim"
    echo "========================================"
    if [ $USE_BUNDLED_LIBS -eq 1 ]; then
        sudo LD_LIBRARY_PATH="$BUNDLED_LIBS:$LD_LIBRARY_PATH" TRUSTM_DATA_PATH="$TRUSTM_DATA_PATH" TRUSTM_LIB_PATH="$TRUSTM_LIB_PATH" "$DEPLOY_DIR/bin/tesaiot_csr_client" $ARGS
    else
        sudo TRUSTM_DATA_PATH="$TRUSTM_DATA_PATH" TRUSTM_LIB_PATH="$TRUSTM_LIB_PATH" "$DEPLOY_DIR/bin/tesaiot_csr_client" $ARGS
    fi
fi
