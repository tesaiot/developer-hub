...
cy_mqtt_connect_info_t connection_info =
{
 ...
 .password = NULL,
 .password_len = 0,
 /* CRITICAL DECISION (2025-10-23): clean_session setting trade-offs
 *
 * clean_session = true:
 * Fresh start every connection (no stale messages from previous sessions)
 * Simpler workflow (request -> response cycle is clean)
 * If disconnect during Protected Update, must restart from beginning
 *
 * clean_session = false:
 * Broker queues messages during disconnect (reliable delivery)
 * Can resume workflow after reconnect
 * Old queued messages from failed attempts are re-delivered
 * Must manually clear session state between test runs
 *
 * CHOSEN: clean_session = true for Protected Update workflow
 * - Protected Update is atomic operation (complete or retry from start)
 * - Stale bundles from previous attempts cause confusion
