# MQTT over QUIC - Python Example

This example demonstrates connecting to TESAIoT Platform using **MQTT over QUIC** with the aioquic library.
...
- **Server-TLS authentication** (username/password + CA verification)
...
| UDP Port | Outbound UDP 14567 must be allowed |
...
sudo apt update && sudo apt install -y python3 python3-pip python3-venv libssl-dev
...
# Edit with your credentials from the bundle
nano .env
```
...
```bash
cp /path/to/downloaded/bundle/ca-chain.pem ./ca-chain.pem
```
...
# Authentication (from bundle)
MQTT_USERNAME=your-device-id-here
MQTT_PASSWORD=your-password-here

# TLS Settings
MQTT_TLS_ENABLED=true
MQTT_TLS_VERSION=1.3
MQTT_CA_CERT=./ca-chain.pem

# Topics
MQTT_TOPIC_TELEMETRY=device/<your-device-id>/telemetry
```
