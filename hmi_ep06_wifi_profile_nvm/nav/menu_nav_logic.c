...
static void menu_nav_set_sidebar_link_style(lv_obj_t *link, bool active)
{
    ...
    bg = active ? lv_color_hex(UI_MENU_SIDEBAR_ACTIVE_BG_HEX)
                : lv_color_hex(UI_MENU_SIDEBAR_ITEM_BG_HEX);
    border = active ? lv_color_hex(UI_MENU_SIDEBAR_ACTIVE_BORDER_HEX)
                    : lv_color_hex(UI_MENU_SIDEBAR_ITEM_BORDER_HEX);
    text = active ? lv_color_hex(UI_MENU_SIDEBAR_ACTIVE_TEXT_HEX)
...
static void menu_nav_apply_active_state(menu_nav_state_t *state)
{
    ...
    menu_nav_set_sidebar_link_style(state->sidebar_link_wifi_scan,
                                    state->current_page == MENU_NAV_PAGE_WIFI_SCAN);
    menu_nav_set_sidebar_link_style(state->sidebar_link_wifi_profile,
                                    state->current_page == MENU_NAV_PAGE_WIFI_PROFILE);
}
...
static void menu_nav_hide_internal_headers(menu_nav_state_t *state)
{
    ...
    sidebar_header = lv_menu_get_sidebar_header(state->menu);
...
/* Keep sidebar page attached and toggle only visibility/width for stability. */
static void menu_nav_apply_sidebar_state(menu_nav_state_t *state)
{
    ...
    if(state->sidebar_expanded) {
        if(sidebar_cont != NULL) {
            ...
            lv_obj_set_width(sidebar_cont, LV_PCT(32));
        }
...
/* Queue page switch to next LVGL cycle to avoid race when sidebar just changed. */
                         ...
                         lv_obj_t *status_label)
{
    ...
    state->sidebar_link_wifi_scan = NULL;
    state->sidebar_link_wifi_profile = NULL;
...
void menu_nav_logic_bind_nav_widgets(menu_nav_state_t *state,
                                     ...
                                     lv_obj_t *sidebar_link_wifi_scan,
                                     lv_obj_t *sidebar_link_wifi_profile)
{
    if(state == NULL) {
        return;
    }
    ...
    state->sidebar_link_wifi_scan = sidebar_link_wifi_scan;
    state->sidebar_link_wifi_profile = sidebar_link_wifi_profile;
...
{
    menu_nav_set_page(state, MENU_NAV_PAGE_WIFI_PROFILE, true);
}
...
void menu_nav_logic_wifi_btn_event_cb(lv_event_t *e)
{
    ...
    state = (menu_nav_state_t *)lv_event_get_user_data(e);
    menu_nav_set_page(state, MENU_NAV_PAGE_WIFI_SCAN, true);
}
