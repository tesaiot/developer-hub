...
static void pdm_probe_logger_task(void *arg)
{
    ...
    for (;;)
    {
        ...
        {
            printf("[EP07][MIC] GET_FAIL rslt=0x%08lx\r\n", (unsigned long)rslt);
            continue;
        }
        ...
        /* Publish latest stereo snapshot for SensorHub Audio tab (non-blocking). */
        mic_presenter_publish_sample(&sample);

        if ((frame_count % PDM_LOG_DECIMATE_FRAMES) == 0U)
        {
            printf("[EP07][MIC] frame=%lu "
...
cy_rslt_t pdm_probe_logger_start(void)
{
    ...
    if (pdPASS != task_rslt)
    {
        printf("[EP07][MIC] TASK_CREATE_FAIL\r\n");
        return CY_RSLT_TYPE_ERROR;
    }

    printf("[EP07][MIC] LOGGER_START ok log_period_ms=100 scale=raw+ui\r\n");
