...
/* Build one full-size content page so each page_xxx behaves consistently. */
...
void ui_menu_navigation_create(void)
{
    ...
    /* Header: left settings icon + centered title. */
    ...
    lv_label_set_text(header_title, "Menu Navigation");
    ...
    /* Top tabs are always visible for simple navigation in class. */
    ...
    lv_obj_t *btn_wifi = create_top_nav_button(top_nav, "WiFi");
    lv_obj_t *btn_display = create_top_nav_button(top_nav, "Display");
    lv_obj_t *btn_device = create_top_nav_button(top_nav, "Info");
    ...
    lv_obj_t *home_page = create_full_content_page(menu,
                                                   "Home",
                                                   "Home",
                                                   "Use top tabs or open sidebar from the Settings icon.",
                                                   "Active style follows current page on both top tabs and sidebar.");

    lv_obj_t *page_wifi = create_full_content_page(menu,
                                                   "WiFi Manager",
                                                   "WiFi Manager",
                                                   "Placeholder for scan/list and connection flow (EP05).",
                                                   "Keep this page for WiFi widgets in next episodes.");

    lv_obj_t *page_display = create_full_content_page(menu,
                                                      "Display Setting",
                                                      "Display Setting",
                                                      "Placeholder for brightness and theme settings.",
                                                      "Use it for slider/switch/dropdown demos.");

    lv_obj_t *page_device = create_full_content_page(menu,
                                                     "Device Info",
                                                     "Device Info",
                                                     "Placeholder for firmware and board information.",
                                                     "Use this as a read-only diagnostics template.");

    /* Sidebar has two parts: menu links + tip card. */
    ...
    lv_obj_t *link_wifi = create_sidebar_link(sidebar_section_nav,
                                              LV_SYMBOL_WIFI,
                                              "WiFi Manager",
                                              menu_nav_logic_wifi_btn_event_cb,
                                              &s_menu_nav_state);

    lv_obj_t *link_display = create_sidebar_link(sidebar_section_nav,
                                                 LV_SYMBOL_SETTINGS,
                                                 "Display Setting",
                                                 menu_nav_logic_display_btn_event_cb,
                                                 &s_menu_nav_state);

    lv_obj_t *link_device = create_sidebar_link(sidebar_section_nav,
                                                LV_SYMBOL_LIST,
                                                "Device Info",
                                                menu_nav_logic_device_btn_event_cb,
                                                &s_menu_nav_state);
    ...
    lv_obj_add_event_cb(btn_wifi, menu_nav_logic_wifi_btn_event_cb, LV_EVENT_CLICKED, &s_menu_nav_state);
    lv_obj_add_event_cb(btn_display, menu_nav_logic_display_btn_event_cb, LV_EVENT_CLICKED, &s_menu_nav_state);
    lv_obj_add_event_cb(btn_device, menu_nav_logic_device_btn_event_cb, LV_EVENT_CLICKED, &s_menu_nav_state);
